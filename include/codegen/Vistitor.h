...
private:
    ...
    void bType(std::shared_ptr<VNodeBase> node);        // 基本类型
    void constDef(std::shared_ptr<VNodeBase> node);     // 常量定义
    void constInitVal(std::shared_ptr<VNodeBase> node); // 常量初值
    void constExp(std::shared_ptr<VNodeBase> node);     // 常量表达式
    void varDecl(std::shared_ptr<VNodeBase> node);      // 变量声明
    void varDef(std::shared_ptr<VNodeBase> node);       // 变量定义
    void initVal(std::shared_ptr<VNodeBase> node);      // 变量初值
    ...
    void exp(std::shared_ptr<VNodeBase> node);          // 表达式
    ...
    void funcType(std::shared_ptr<VNodeBase> node);     // 函数类型
